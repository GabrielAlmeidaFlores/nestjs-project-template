...
import { GetRuralTimelineAnalysisLegalProceedingQueryResult } from '@module/customer/analysis-tool/module/rural-timeline-analysis/domain/repository/rural-timeline-analysis/query/result/get-rural-timeline-analysis-with-relations.query.result';
...
export class GetRuralTimelineAnalysisLegalProceedingQueryResultAutoMapperProfile {
  ...
  private createMap(mapper: Mapper): void {
    createMap(
      ...
      forMember(
        (destination) => destination.legalProceedingNumber,
        mapFrom((source) => source.legalProceedingNumber),
      ),
    );
  }
}
