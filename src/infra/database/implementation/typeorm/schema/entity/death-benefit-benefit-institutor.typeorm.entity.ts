...
@Entity({ name: 'death_benefit_benefit_institutor' })
export class DeathBenefitBenefitInstitorTypeormEntity extends BaseTypeormEntity {
  ...
  @Column({ name: 'sex', type: 'simple-enum', enum: GenderEnum, nullable: true })
  ...
  @Column({ name: 'retirement_benefit_number', type: 'varchar', length: 255, nullable: true })
  public retirementBenefitNumber: string | null;

  @ManyToOne(
    () => DeathBenefitTypeormEntity,
    (entity) => entity.deathBenefitBenefitInstitutor,
    { nullable: true },
  )
  @JoinColumn({ name: 'death_benefit_id' })
  public deathBenefit?: DeathBenefitTypeormEntity | null;

  protected override readonly _type = DeathBenefitBenefitInstitorTypeormEntity.name;
}
