...
import { ConversationStatusTypeEnum } from '@module/ai/domain/schema/entity/conversation/enum/conversation-status-type-enum';
...
} from '@module/ai/dto/response/send-message-to-conversation.response.dto';
import { ConversationNotFoundError } from '@module/ai/error/conversation-not-found.error';
...
import {
  ...
  JsonValueType,
  McpUseCase,
} from '@module/ai/mcp/use-case/mcp.use-case';
...
export class SendMessageToConversationUseCase {
  ...
  public async execute(
    ...
    const maybeFiles =
      ((dto as Record<string, unknown>).files as unknown[]) ?? [];
    ...
    if (Array.isArray(maybeFiles) && maybeFiles.length > 0) {
      for (const f of maybeFiles) {
        ...
        const buf: Buffer | undefined = (fileObj.buffer ?? fileObj.data) as
          | Buffer
          | undefined;
        const mime: string | undefined = (fileObj.mimetype ??
          fileObj.mimeType ??
          fileObj.mime) as string | undefined;
...
`.trim();
  ...
  private extractInssBenefitNumbers(text: string): string[] {
    const MIN_BENEFIT_NUMBER_LENGTH = 8;
    const MAX_BENEFIT_NUMBER_LENGTH = 12;
    ...
    const filtered = matches
      ...
      .filter(
        (m) =>
          m.length >= MIN_BENEFIT_NUMBER_LENGTH &&
          m.length <= MAX_BENEFIT_NUMBER_LENGTH,
      );
