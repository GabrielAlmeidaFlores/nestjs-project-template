import { SupportAttendantListItemResponseDto } from '@module/admin/service-desk/dto/response/support-attendant-list-item.response.dto';
import { ResponseDto } from '@shared/api/util/decorator/class/dto-specification/response-dto.decorator';
import { ResponseDtoObjectProperty } from '@shared/api/util/decorator/property/dto-property/response/response-dto-object-property/response-dto-object-property.decorator';
import { ListDataResponseDto } from '@shared/api/util/dto/response/list-data.response.dto';

@ResponseDto()
export class ListSupportAttendantsResponseDto extends ListDataResponseDto<SupportAttendantListItemResponseDto> {
  @ResponseDtoObjectProperty(() => SupportAttendantListItemResponseDto, {
    isArray: true,
  })
  public override resource: SupportAttendantListItemResponseDto[];

  protected override readonly _type = ListSupportAttendantsResponseDto.name;
}
