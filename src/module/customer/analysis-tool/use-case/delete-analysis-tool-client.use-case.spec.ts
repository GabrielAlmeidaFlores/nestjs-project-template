...
describe(DeleteAnalysisToolClientUseCase.name, () => {
  ...
  it('deve deletar um cliente da ferramenta de análise com sucesso', async () => {
  ...
  it('deve lançar OrganizationMemberNotFoundError quando o membro da organização não for encontrado', async () => {
    const sessionData = buildSessionData();
    const orgSessionData = buildOrganizationSessionData();
    const analysisToolClientId = new AnalysisToolClientId();
