...
@Injectable()
export class GenerateAnalysisToolClientInterviewFormDocumentUseCase {
  protected readonly _type = GenerateAnalysisToolClientInterviewFormDocumentUseCase.name;
  ...
  private buildHtml(
    clientName: string,
    form: GetAnalysisToolClientInterviewFormQueryResult | null,
  ): string {
    const yes = 'Sim';
    const no = 'Não';
    const na = 'N/A';
    ...
    return `
<html>
<head><meta charset="UTF-8"><title>Ficha de Entrevista</title>
<style>
...
<table>
  ...
  <tr><td>Casado(a) / União Estável</td><td>${bool(f?.isMarried)}</td></tr>
