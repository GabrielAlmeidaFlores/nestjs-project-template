...
@Injectable()
export class ListGeneralUrbanRetirementReviewTimeAcceleratorUseCase {
  protected readonly _type =
    ListGeneralUrbanRetirementReviewTimeAcceleratorUseCase.name;
  ...
  public async execute(
    ...
    dto: ListGeneralUrbanRetirementReviewTimeAcceleratorRequestDto,
    ...
    const resource = acceleratorTimes.map((item) =>
      GetGeneralUrbanRetirementReviewTimeAcceleratorResponseDto.build({
        ...item,
    ...
    return ListGeneralUrbanRetirementReviewTimeAcceleratorResponseDto.build({
      resource,
      total,
      page: 1,
      limit: totalItems || 1,
  ...
  private totalWorkPeriodFromArray(
    items: GetGeneralUrbanRetirementReviewTimeAcceleratorQueryResult[],
  ): { years: number; months: number; days: number } {
    let totalYears = 0;
    let totalMonths = 0;
    let totalDays = 0;
