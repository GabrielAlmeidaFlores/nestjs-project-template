...
export class UpdateRuralTimelineAnalysisCnisContributionPeriodUseCase {
  ...
  public async execute(
    ...
    const existingPeriod = await this.fetchPeriodOrThrow(contributionPeriodId);

    this.validatePeriodBelongsToTimeline(
      existingPeriod,
      ruralTimelineAnalysisId,
    );

    const cnisDocumentLocation =
      await this.uploadCnisDocumentIfProvided(dto, existingPeriod);
  ...
  private async fetchPeriodOrThrow(
    contributionPeriodId: RuralTimelineAnalysisCnisContributionPeriodId,
  ): Promise<RuralTimelineAnalysisCnisContributionPeriodEntity> {
    const period =
  ...
  private validatePeriodBelongsToTimeline(
    period: RuralTimelineAnalysisCnisContributionPeriodEntity,
    ruralTimelineAnalysisId: RuralTimelineAnalysisId,
  ): void {
    if (period.ruralTimelineId?.toString() !== ruralTimelineAnalysisId.toString()) {
      throw new RuralTimelineAnalysisCnisContributionPeriodNotFoundError();
    }
  }
  ...
  private async uploadCnisDocumentIfProvided(
    dto: UpdateRuralTimelineAnalysisCnisContributionPeriodRequestDto,
    existingPeriod: RuralTimelineAnalysisCnisContributionPeriodEntity,
  ): Promise<string | null> {
    if (dto.cnisDocument === undefined) {
      return existingPeriod.cnisDocument;
    }
  ...
  private async updatePeriod(
    ...
    const updatedEntity = new RuralTimelineAnalysisCnisContributionPeriodEntity({
      ...
      employmentRelationshipSource:
        dto.employmentRelationshipSource ?? existingPeriod.employmentRelationshipSource,
      ...
      qualifyingPeriod: dto.qualifyingPeriod ?? existingPeriod.qualifyingPeriod,
      status: dto.status ?? existingPeriod.status,
      averageContributionAmount:
        dto.averageContributionAmount ?? existingPeriod.averageContributionAmount,
      contributionAdjustmentIntent:
        dto.contributionAdjustmentIntent ?? existingPeriod.contributionAdjustmentIntent,
      externalSupplementationIntent:
        dto.externalSupplementationIntent ?? existingPeriod.externalSupplementationIntent,
      shouldConsiderPeriod:
        dto.shouldConsiderPeriod ?? existingPeriod.shouldConsiderPeriod,
      shouldConsiderLastRemunerationAsExitDate:
        dto.shouldConsiderLastRemunerationAsExitDate ??
        existingPeriod.shouldConsiderLastRemunerationAsExitDate,
      cnisDocument: cnisDocumentLocation,
      impactAnalysis: existingPeriod.impactAnalysis,
