...
@Injectable()
export class UpdateDeathBenefitPeriodUseCase {
  protected readonly _type = UpdateDeathBenefitPeriodUseCase.name;
  ...
  public async execute(
    ...
    dto: UpdateDeathBenefitPeriodRequestDto,
    ...
    const existingPeriods = existingDeathBenefit.deathBenefitPeriod ?? [];
    const periodIds: DeathBenefitPeriodId[] = [];
    ...
    for (const existingPeriod of existingPeriods) {
      ...
      transactions.push(
        this.deathBenefitPeriodCommandRepositoryGateway.deleteDeathBenefitPeriod(
          existingPeriod.id,
        ),
      );
    }
    ...
    return UpdateDeathBenefitPeriodResponseDto.build({ deathBenefitId });
  }
}
