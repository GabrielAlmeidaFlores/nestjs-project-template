...
export class AnalyzeGeneralUrbanRetirementGrantPppUseCase {
  ...
  public constructor(
    ...
    @Inject(GeneralUrbanRetirementGrantSpecialPeriodCommandRepositoryGateway)
    private readonly generalUrbanRetirementGrantSpecialPeriodCommandRepositoryGateway: GeneralUrbanRetirementGrantSpecialPeriodCommandRepositoryGateway,
  ...
  public async execute(
    ...
    dto: AnalyzeGeneralUrbanRetirementGrantPppRequestDto,
    ...
    const generalUrbanRetirementGrantSpecialPeriod =
      new GeneralUrbanRetirementGrantSpecialPeriodEntity({
        response: result,
        generalUrbanRetirementGrant: generalUrbanRetirementGrantEntity,
      });

    const generalUrbanRetirementGrantSpecialPeriodSaved =
      this.generalUrbanRetirementGrantSpecialPeriodCommandRepositoryGateway.createGeneralUrbanRetirementGrantSpecialPeriod(
        generalUrbanRetirementGrantSpecialPeriod,
      );

    const transactions = await this.baseTransactionRepositoryGateway.execute([
      generalUrbanRetirementGrantSpecialPeriodSaved,
      consumeCreditTransaction,
    ]);

    await transactions.commit();

    return AnalyzeGeneralUrbanRetirementGrantPppResponseDto.build({
      generalUrbanRetirementGrantSpecialPeriodId:
        generalUrbanRetirementGrantSpecialPeriod.id,
