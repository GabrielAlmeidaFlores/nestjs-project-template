...
import { RetirementPermanentDisabilityRejectionCompleteAnalysisDownloadNotFoundError } from '@module/customer/analysis-tool/module/retirement-permanent-disability-rejection/error/retirement-permanent-disability-rejection-complete-analysis-download-not-found.error';
...
@Injectable()
export class DownloadRetirementPermanentDisabilityRejectionCompleteAnalysisUseCase {
  protected readonly _type =
    DownloadRetirementPermanentDisabilityRejectionCompleteAnalysisUseCase.name;
  ...
  public async execute(
    ...
    const completeAnalysisMarkdown =
      existing.retirementPermanentDisabilityRejectionResult
        ?.completeAnalysisDownload ?? null;

    if (completeAnalysisMarkdown === null) {
      throw new RetirementPermanentDisabilityRejectionCompleteAnalysisDownloadNotFoundError();
    }

    const htmlContent = await this.exportDocumentGateway.convertMarkdownToHtml(
      completeAnalysisMarkdown,
    );

    return this.exportDocumentGateway.downloadFileAsStreamable(
      htmlContent,
      format,
      'analise_completa_indeferimento_aposentadoria_incapacidade_permanente',
    );
  }
}
