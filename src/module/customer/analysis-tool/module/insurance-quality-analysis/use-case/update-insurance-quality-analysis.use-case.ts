  ...
  public async execute(
    ...
    const cnisDocument =
      dto.cnisDocument !== undefined
        ? await this.fileProcessorGateway.uploadFile(
            dto.cnisDocument,
            insuranceQualityAnalysisQueryResult.cnisDocument ?? undefined,
          )
    ...
    const ruralDocument =
      dto.ruralDocument !== undefined
        ? await this.fileProcessorGateway.uploadFile(
            dto.ruralDocument,
            insuranceQualityAnalysisQueryResult.ruralDocument ?? undefined,
          )
        : insuranceQualityAnalysisQueryResult.ruralDocument;

    const complementaryDocument =
      dto.complementaryDocument !== undefined
        ? await this.fileProcessorGateway.uploadFile(
            dto.complementaryDocument,
