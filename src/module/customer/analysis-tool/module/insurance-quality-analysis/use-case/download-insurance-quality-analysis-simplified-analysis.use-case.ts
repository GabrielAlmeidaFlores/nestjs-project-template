...
import { InsuranceQualityAnalysisDoesNotContainSimplifiedAnalysisError } from '@module/customer/analysis-tool/module/insurance-quality-analysis/error/insurance-quality-analysis-does-not-contain-simplified-analysis.error';
...
@Injectable()
export class DownloadInsuranceQualityAnalysisSimplifiedAnalysisUseCase {
  protected readonly _type =
    DownloadInsuranceQualityAnalysisSimplifiedAnalysisUseCase.name;
  ...
  public async execute(
    ...
    if (analysisSummary === null) {
      ...
      analysisSummary =
        await this.analysisProcessorGateway.getInsuranceQualityAnalysisCompleteAnalysis(
          completePromptResponse.prompt,
          [clientDataBuffer, ...documentBuffers],
        );

      if (analysisSummary === null) {
        throw new InsuranceQualityAnalysisDoesNotContainSimplifiedAnalysisError();
      }
    ...
    const simplifiedAnalysis =
      await this.analysisProcessorGateway.getInsuranceQualityAnalysisSimplifiedAnalysis(
        promptResponse.prompt,
        [Buffer.from(analysisSummary, 'utf-8')],
      );
