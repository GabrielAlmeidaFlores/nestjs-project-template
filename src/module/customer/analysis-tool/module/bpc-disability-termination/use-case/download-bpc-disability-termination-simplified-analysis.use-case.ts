...
import { BpcDisabilityTerminationDoesNotContainSimplifiedAnalysisError } from '@module/customer/analysis-tool/module/bpc-disability-termination/error/bpc-disability-termination-does-not-contain-simplified-analysis.error';
...
@Injectable()
export class DownloadBpcDisabilityTerminationSimplifiedAnalysisUseCase {
  protected readonly _type =
    DownloadBpcDisabilityTerminationSimplifiedAnalysisUseCase.name;
  ...
  public async execute(
    ...
    if (bpcDisabilityTerminationResult === null) {
      throw new BpcDisabilityTerminationDoesNotContainCompleteAnalysisError();
    }

    if (bpcDisabilityTerminationResult.completeAnalysisDownload === null) {
      throw new BpcDisabilityTerminationDoesNotContainCompleteAnalysisError();
    }

    let responseAi = bpcDisabilityTerminationResult.simplifiedAnalysis;

    if (responseAi === null) {
      const simplifiedAnalysis =
        await this.analysisProcessorGateway.getBpcDisabilityTerminationSimplifiedAnalysis(
          promptResponse.prompt,
          [
            Buffer.from(
              bpcDisabilityTerminationResult.completeAnalysisDownload,
              'utf-8',
            ),
          ],
        );

      const updatedResultEntity = new BpcDisabilityTerminationResultEntity({
      ...
      const updateResultTransaction =
        this.bpcDisabilityTerminationResultCommandRepositoryGateway.updateBpcDisabilityTerminationResult(
          bpcDisabilityTerminationResult.id,
          updatedResultEntity,
        );
    ...
    if (responseAi === null) {
      throw new BpcDisabilityTerminationDoesNotContainSimplifiedAnalysisError();
    }

    return this.exportDocumentGateway.downloadFileAsStreamable(
      responseAi,
      format,
      'analise_simplificada_cessacao_bpc_pcd',
    );
  }
}
