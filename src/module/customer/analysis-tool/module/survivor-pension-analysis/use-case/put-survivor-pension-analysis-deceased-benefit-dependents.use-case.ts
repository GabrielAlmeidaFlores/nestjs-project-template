...
@Injectable()
export class PutSurvivorPensionAnalysisDeceasedBenefitDependentsUseCase {
  protected readonly _type =
    PutSurvivorPensionAnalysisDeceasedBenefitDependentsUseCase.name;
  ...
  public async execute(
    ...
    dto: PutSurvivorPensionAnalysisDeceasedBenefitDependentsRequestDto,
    ...
    const entities = dto.dependents.map(
      (dependentDto) =>
        new SurvivorPensionAnalysisDeceasedBenefitDependentsEntity({
          survivorPensionAnalysisId,
          ...(dependentDto.dependentFullName !== undefined && {
            dependentFullName: dependentDto.dependentFullName,
          }),
          ...(dependentDto.dependencyClassificationLevel !== undefined && {
            dependencyClassificationLevel:
              dependentDto.dependencyClassificationLevel,
          }),
          ...(dependentDto.type !== undefined && { type: dependentDto.type }),
          ...(dependentDto.gender !== undefined && {
            gender: dependentDto.gender,
          }),
          ...(dependentDto.dateOfBirth !== undefined && {
            dateOfBirth: dependentDto.dateOfBirth,
          }),
          ...(dependentDto.hasDisabilityOrInvalidity !== undefined && {
            hasDisabilityOrInvalidity: dependentDto.hasDisabilityOrInvalidity,
          }),
          ...(dependentDto.unionCommencementDate !== undefined && {
            unionCommencementDate: dependentDto.unionCommencementDate,
          }),
        }),
    );

    const txn = await this.baseTransactionRepositoryGateway.execute([
      this.survivorPensionAnalysisDeceasedBenefitDependentsCommandRepositoryGateway.deleteAllBySurvivorPensionAnalysisId(
        survivorPensionAnalysisId,
      ),
      ...entities.map((entity) =>
    ...
    return PutSurvivorPensionAnalysisDeceasedBenefitDependentsResponseDto.build(
      { survivorPensionAnalysisId },
    );
  }
}
