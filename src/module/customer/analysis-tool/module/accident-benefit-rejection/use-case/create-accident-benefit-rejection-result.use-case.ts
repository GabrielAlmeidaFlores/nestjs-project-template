...
import { AccidentBenefitRejectionResultNotFoundError } from '@module/customer/analysis-tool/module/accident-benefit-rejection/error/accident-benefit-rejection-result-not-found.error';
import { InvalidAccidentBenefitRejectionResultJsonError } from '@module/customer/analysis-tool/module/accident-benefit-rejection/error/invalid-accident-benefit-rejection-result-json.error';
...
@Injectable()
export class CreateAccidentBenefitRejectionResultUseCase {
  protected readonly _type = CreateAccidentBenefitRejectionResultUseCase.name;
  ...
  public async execute(
    ...
    if (existingResult === null) {
      throw new AccidentBenefitRejectionResultNotFoundError();
    }
    ...
    const completeAnalysisResponse =
      await this.analysisProcessorGateway.getAccidentBenefitRejectionCompleteAnalysis(
    ...
    return CreateAccidentBenefitRejectionResultResponseDto.build({
      accidentBenefitRejectionCompleteAnalysis: parsedResult,
    });
  }
  ...
  private parseResultAnalysis(
    ...
    if (!this.isResultAnalysis(parsed)) {
      throw new InvalidAccidentBenefitRejectionResultJsonError();
    }
  ...
  private isResultAnalysis(
    value: unknown,
  ): value is AccidentBenefitRejectionResultInterface {
  ...
  private buildRejectionDataBuffer(
    ...
    const data = {
      ...
      workPeriodsEarningsHistory: (
        rejection.accidentBenefitRejectionWorkPeriodEarningsHistory ?? []
      ).map((item) => ({
        ...
        ...(item.accidentBenefitRejectionWorkPeriodId !== null && {
          accidentBenefitRejectionWorkPeriodId:
            item.accidentBenefitRejectionWorkPeriodId.toString(),
        }),
      })),
    };
