...
export class CreateMiniAdvisorResultUseCase {
  ...
  public async execute(
    ...
    const predefinedMessages =
      miniAdvisorAnalysisPredefinedMessages[aiResult.chosenAnalysis];
  ...
  private parseAiResponse(raw: string | null): MiniAdvisorAiResultInterface {
    if (raw === null) {
      return {
        chosenAnalysis:
    ...
    try {
      const parsed = JSON.parse(raw) as Partial<MiniAdvisorAiResultInterface>;
