...
export class AddGeneralUrbanRetirementFlow1772046928078 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`general_urban_retirement_analysis_period_disability\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`type\` enum ('total', 'parcial', 'nenhum') NOT NULL, \`degree\` enum ('leve', 'moderado', 'grave') NOT NULL, \`start_date\` date NOT NULL, \`end_date\` date NOT NULL, \`category\` enum ('mental_ou_intelectual', 'fisica', 'sensorial') NOT NULL, \`daily_impact\` text NOT NULL, \`description\` text NOT NULL, \`lawyer_observations\` text NULL, \`cid_id\` varchar(36) NULL, \`general_urban_retirement_analysis_period_id\` varchar(36) NULL, UNIQUE INDEX \`REL_a258ae7dbcec4aedd5d9e83e16\` (\`general_urban_retirement_analysis_period_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`general_urban_retirement_analysis_period_document\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`document_type\` enum ('ctc_document', 'ppp', 'ctps', 'ltcat', 'judicial', 'medico', 'outros', 'outros_medicos') NOT NULL DEFAULT 'outros', \`document\` varchar(255) NOT NULL, \`general_urban_retirement_analysis_period_special_time_id\` varchar(36) NULL, \`general_urban_retirement_analysis_period_disability_id\` varchar(36) NULL, \`general_urban_retirement_analysis_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
