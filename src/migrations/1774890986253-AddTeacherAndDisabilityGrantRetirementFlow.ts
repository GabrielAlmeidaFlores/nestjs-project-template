...
export class AddTeacherAndDisabilityGrantRetirementFlow1774890986253
  implements MigrationInterface
{
  name = 'AddTeacherAndDisabilityGrantRetirementFlow1774890986253';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`teacher_retirement_planning\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`federative_entity\` enum('state','municipality','union','federal_district') NOT NULL, \`state\` varchar(2) NULL, \`municipality\` varchar(255) NULL, \`analysis_name\` varchar(255) NULL, \`current_position\` varchar(255) NULL, \`activity_type\` enum('classroom_only','classroom_and_pedagogical_function','no_teaching_activity') NOT NULL, \`public_service_start_date\` date NOT NULL, \`career_start_date\` date NOT NULL, \`administrative_process_analysis\` longtext NULL, \`teacher_retirement_planning_result_id\` varchar(36) NULL, UNIQUE INDEX \`REL_teacher_retirement_planning_result\` (\`teacher_retirement_planning_result_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    ...
    const [teacherColRows] = await queryRunner.query(
      `SELECT COUNT(*) as cnt FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'analysis_tool_record' AND COLUMN_NAME = 'teacher_retirement_planning_id'`,
    );

    if (teacherColRows.cnt === 0) {
      await queryRunner.query(
        `ALTER TABLE \`analysis_tool_record\` ADD \`teacher_retirement_planning_id\` varchar(36) NULL`,
      );
    ...
    await queryRunner.query(
      `ALTER TABLE \`analysis_tool_record\` CHANGE \`type\` \`type\` enum ('analise_rapida_cnis', 'planejamento_previdenciario_rgps', 'planejamento_aposentadoria_rpps', 'planejamento_previdenciario_professor', 'atividade_especial', 'analise_caso_judicial', 'analise_procedimento_administrativo_inss', 'gerador_perguntas_medicas', 'analise_geradora_objecao_laudo_medico_social', 'gerador_discurso', 'avaliacao_deficiencia_para_bpc', 'analise_renda_per_capita_para_bpc', 'analise_linha_tempo_rural', 'gerador_perguntas_audiencia', 'analise_qualidade_segurado', 'planejamento_aposentadoria_para_deficiente', 'concessao_aposentadoria_urbana_geral', 'analise_aposentadoria_urbana_geral', 'aposentadoria_categoria_especial', 'concessao_aposentadoria_para_deficiente') NOT NULL`,
    );

    await queryRunner.query(
      `ALTER TABLE \`analysis_tool_record\` ADD UNIQUE INDEX \`IDX_drpg_analysis_tool_record\` (\`disability_retirement_planning_grant_id\`)`,
    );
    ...
    const [relIndexRows] = await queryRunner.query(
      `SELECT COUNT(*) as cnt FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'analysis_tool_record' AND INDEX_NAME = 'REL_de48ccd1c2a2087b6f247f298d'`,
    );

    if (relIndexRows.cnt === 0) {
      await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`disability_retirement_planning_grant_document\` ADD CONSTRAINT \`FK_drpg_document\` FOREIGN KEY (\`disability_retirement_planning_grant_id\`) REFERENCES \`disability_retirement_planning_grant\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );

    await queryRunner.query(
      `CREATE UNIQUE INDEX \`REL_drpg_analysis_tool_record\` ON \`analysis_tool_record\` (\`disability_retirement_planning_grant_id\`)`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX \`REL_drpg_analysis_tool_record\` ON \`analysis_tool_record\``,
    );

    await queryRunner.query(
      `ALTER TABLE \`disability_retirement_planning_grant_document\` DROP FOREIGN KEY \`FK_drpg_document\``,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`analysis_tool_record\` DROP INDEX \`IDX_drpg_analysis_tool_record\``,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`analysis_tool_record\` DROP COLUMN \`teacher_retirement_planning_id\``,
    );
    ...
    await queryRunner.query(`DROP TABLE \`disability_retirement_planning_grant\``);
