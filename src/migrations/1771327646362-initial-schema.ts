...
export class InitialSchema1771327646362 implements MigrationInterface {
  name = 'InitialSchema1771327646362';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`analysis_tool_client_legal_proceeding\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`legal_proceeding_number\` varchar(100) NOT NULL, \`type\` varchar(100) NULL, \`status\` enum ('IN_PROGRESS', 'COMPLETED') NULL, \`last_updated\` date NULL, \`deadline\` date NULL, \`analysis_tool_client_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
